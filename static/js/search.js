          ...
          result = JSON.parse(res)
            ...
            if (result.data.length > 0) {
              ...
              $.each(arr, function(i, item){ 
                  html +=  '<section>'
                  + '<div class="content cursor-pointer box-shadow">'
                  + '<img width="110" height="96" src="' + item.icon + '" class="attachment-post-thumbnail size-post-thumbnail wp-post-image" alt="' + item.keyname + '" decoding="async" />'
                  + '<h1><a href="' + item.path_url + '" title="' + item.keyname + '">' + item.keyname + '</a></h1>'
                  + '<div class="date-on-tax">'
                  + '<span>' + item.update_str + ': </span>'
                  + '<span class="limit-line">' + item.datetime + '</span></div></div></section>';
              }); 
